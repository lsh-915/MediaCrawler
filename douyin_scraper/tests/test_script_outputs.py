...
def test_script_raw_default_limit_follows_max_videos_per_keyword(
    tmp_path: Path,
) -> None:
    scraper = _scraper(tmp_path)
    scraper.config.max_videos_per_keyword = 2
    sources = tmp_path / "script_sources.jsonl"
    _write_jsonl(
        sources,
        [
            _source("raw-1", video_url="https://example.com/raw-1.mp4"),
            _source("raw-2", video_url="https://example.com/raw-2.mp4"),
            _source("raw-3", video_url="https://example.com/raw-3.mp4"),
        ],
    )
    ...
    by_id = {row["aweme_id"]: row for row in rows}
    assert by_id["raw-1"]["download_status"] == "success"
    assert by_id["raw-2"]["download_status"] == "success"
